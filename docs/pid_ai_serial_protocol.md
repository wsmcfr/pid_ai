...
`loop_id` 和 `loop_name` 必须使用安全 ASCII 标识符字符集 `A-Za-z0-9_.:-`，并保持非空。不要发送空格、逗号、回车、换行、引号、`<`、`>`、斜杠或非 ASCII 文本；上位机必须在这些字段进入 loop 状态、命令匹配或页面渲染前拒绝坏帧。
...
`{CFGX}` 的 `loop_id` 和 `loop_name` 使用与 `{PIDX}` 相同的安全 ASCII 标识符规则。若板端需要显示中文或更长描述，应通过独立低频元数据协议传输，不应放进用于实时路由和 ACK 匹配的字段。
...
文本字段必须为非空安全 ASCII 标识符，字符集为 `A-Za-z0-9_.:-`。二进制接收端必须先校验长度和 CRC，再把 payload 解析为 typed frame；CRC 错误或字段非法时不得覆盖最新有效状态。

CRC 正确不代表 payload 数值有效。二进制 payload 中的所有 `float32` 字段仍必须经过 finite 校验，`NaN`、`+Inf`、`-Inf` 都必须视为非法帧；不能把它们写入 `latest_pid`、`latest_cfg`、`loops`、评分窗口或自动调参状态。
