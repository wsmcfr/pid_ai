...
| 自动调参状态机 | `{PID}` / `{CFG}` 或 `{PIDX}` / `{CFGX}`、`{SENS}`、`{ACK}`、`{ERR}` | `autotune`、`scores`、`rollback_history`、可选 `{CMD}SET_PID` / `{CMD}SET_PIDX` | `AutoTuneController` 按 profile 执行安全门槛、ACK 等待、评分和回滚判断 |
...
配置 dashboard 内置自动调参状态机。默认关闭；只有 `enabled=true` 且 `mode="auto-tune"` 时才会自动发送 `SET_PID` / `SET_PIDX` 或回滚命令。
...
| `profile` | string | 否 | `single-loop` 使用 `{PID}/{CFG}` 和 `SET_PID`；`multi-loop` / `line-car-cascade` 使用 `{PIDX}/{CFGX}` 和 `SET_PIDX` |
| `loop_order` | string/array | 否 | 通用多环按内环到外环的 loop 顺序，例如 `motor_speed,angle,position`；空值按发现顺序或 profile 预设 |
| `conservative_loops` | string/array | 否 | 需要半步 `kp` 的保守 loop，例如 `position,line_outer` |
| `line_safety_enabled` | boolean/null | 否 | 是否把 `{SENS}.line_lost=1` 作为停止门槛；null 使用 profile 默认值 |
...
| 启用 line safety 且 `{SENS}` 中 `line_lost=1` | 自动调参进入停止状态 | `autotune.last_action.type=abort`；通用 `multi-loop` 默认不启用，`line-car-cascade` 默认启用 |
...
| `auto-tune` 模式收到 ACK 后新遥测少于 `min_post_ack_samples` | 等待 post-ACK 窗口 | 不提前 keep/rollback |
...
| post-ACK 评分变差 | 单环生成旧参数 `SET_PID`，串级生成旧参数 `SET_PIDX` | `rollback_history` 增加，并记录回滚命令 |
...
| Bad | auto-tune 发出 `SET_PID` / `SET_PIDX` 后串口保持静默 | `/api/status` 轮询或读线程空闲 tick 仍推进 ACK timeout 并进入 `ABORT` |
